...
NAND 每一页除了数据区还有一块**带外区（Out-Of-Band，OOB）**，也叫 spare area。
串口上那行就是它：
...
每 2048 字节数据配 128 字节 OOB。**`dd if=/dev/mtdN` 只读数据区，不含 OOB**；网页
恢复页的 `/dump`（走 `mtd_read()`）也一样。两边格式因此完全一致，这也是它们能互
换的原因。

OOB 里装三样东西，没有一样是备份该负责的：

| 内容 | 谁写的 | 丢了要紧吗 |
| --- | --- | --- |
| ECC 校验码 | 控制器每写一页自动算 | **不要紧**，写回去时重新生成 |
| 出厂坏块标记 | 厂商烧在坏块 OOB 的第 0 字节 | 见下 |
| 文件系统元数据 | 某些 NAND 文件系统会用 | 这台机器上没有 |

**值钱的东西全在数据区。** `ri`（MAC、序列号）、`bosa`（光模块校准）、`romfile`
（SN、PON 认证）都是普通分区内容；UBI 自己的 EC 头与 VID 头也写在数据区，不在
OOB。所以一份不含 OOB 的备份，出厂数据一个字节都没少。

**坏块标记不用备份，因为你是刷回同一块颗粒** —— 它自己的标记本来就还在自己的
OOB 里，没人动过，写入时 `mtd` 层照样跳。真要连 OOB 一起存（`nanddump -o`），那
份镜像反而没法用普通 `mtd write` 写回去。

> ⚠️ 但有一个方向是危险的：**`mtd erase` 连 OOB 一起擦**，出厂坏块标记就在 OOB
> 第 0 字节。U-Boot 的 `mtd erase` 对坏块的处理是「把 BBT 条目摘掉再照擦不误」，
> 那块从此被当好块用，以后往里写的东西会静静坏掉。这就是为什么「重建 UBI」那条
> 路要慎用，也是为什么坏块检查（上一步）要在备份阶段就做完。
