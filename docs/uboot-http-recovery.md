...
| 设备详情 | — | `GET /info` 返回 JSON：设备树 `model` / `compatible`、DRAM、MTD 几何与分区、MAC、U-Boot 版本、UBI 卷表（含有没有 `fip` 卷）。卷表按卷名排序，ID 列是 UBI 卷号（按创建先后分配，不同迁移路径得到的号不同）；卷没有固定物理地址，所以不列。「健康检查」（`GET /check`）与「串口日志」（`GET /log`）是侧栏里另外两页，见[下一节](#030拦截横幅体检日志) |
| 关于 | — | 静态 |
