...
* MD、MF、TF 都走这个钩子。ZN504XG-D 没有 `ri` 卷，出厂 MAC 在 `factory` 卷 `0x141024` 处（12 个十六进制字符）：这个卷是原厂 `reservearea` 分区的原样拷贝，与 pbs05/ponwrt 的同名卷通用，没写入时同样退回固化的随机 MAC。HG5382A 也没有 `ri` 卷，出厂 MAC 在 1 MiB 的 `factory` 卷 `0x2000` 处（6 字节二进制）：这个卷由 pbs05 的 [fiberhome-factory](https://github.com/pbs05/fiberhome-factory) 从原厂备份生成，与 pbs05/ponwrt 的同名卷通用
