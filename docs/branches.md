...
* **U-Boot**（`package/boot/uboot-airoha/`）：Airoha Web U-Boot 网页救砖（`202` 挂钩 + `src/net/httpd.c`）、TCP 发送窗口（`204`，整片下载在 Windows 上只有 30 KB/s）、DRAM 容量探测（`206` / `310`）、复旦颗粒（`120` / `121`）、菜单环境刷新（`210` 挂钩 + `src/arch/arm/mach-airoha/bootmenu-refresh.c`）、四款机型的 defconfig 与默认环境（`files/web-uboot/`，构建时生成）
