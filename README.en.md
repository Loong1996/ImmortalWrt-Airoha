...
2. `Actions → ImmortalWrt-Airoha → Run workflow`. The model defaults to `all` (every model in parallel). A single model is `xg-040g-md` / `xg-040g-mf` / `xg-040g-tf` / `zn504xg-d` / `hg5382a` / `xr1710g` (U-Boot only). Then pick a variant. Memory size defaults to [auto](docs/variants.md#内存容量). Leave it.
